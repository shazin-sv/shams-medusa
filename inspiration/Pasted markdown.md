...
The store's visual hierarchy is ruthlessly simple. Photography does the heavy lifting, supported by short, punchy headlines in Optimistic Medium and body text that stays brief and scannable. Calls to action are pill-shaped, unmistakable, and always Shams Yellow. There is no visual noise, no decoration for decoration's sake — every element either sells or navigates.
...
- Pill-shaped CTAs in saturated yellow create unmistakable action points
...
- **Shams Yellow** (`#FFC107`): Primary CTA background, interactive links, action-driving elements throughout the store
- **Shams Yellow Hover** (`#FFA000`): Darkened yellow for hover states on primary buttons
- **Shams Yellow Pressed** (`#FF8F00`): Deepest yellow for active/pressed button states
- **Shams Yellow Light** (`#FFECB3`): Lighter yellow variant used on dark backgrounds for CTAs
- **Shams Amber** (`#FFB300`): Legacy accent inherited from brand palette, used for deemphasized button text and badges
...
- **Shams Orange** (`#FF6F00`): Deep orange accent for Shams Tools brand
- **Shams Gold** (`#FFD54F`): Soft gold for hero backgrounds
- **Shams Cream** (`#FFF3E0`): Secondary warm surface tint
...
- **Baby Yellow** (`#FFF8E1`): Highlight background, subtle yellow tint for informational areas
...
- **Yellow Infinity Gradient**: The Shams symbol uses a yellow-to-orange gradient on brand materials, though the store uses flat yellow
...
- CTA: Shams Yellow pill button, right-aligned
...
- Don't use Facebook Blue (`#1877F2`) as a primary CTA color — use Shams Yellow (`#FFC107`) instead
...
- Primary CTA: Shams Yellow (`#FFC107`)
- Background: White (`#FFFFFF`)
...
- "Create a product hero section with a full-width cinematic image, `linear-gradient(rgba(0,0,0,0), rgba(0,0,0,0.6))` text overlay, Optimistic-style 64px/500 white headline, and a Shams Yellow (`#FFC107`) pill button (100px radius, 10px 22px padding)"
...
- "Build a sticky navigation bar with white background, `rgba(241, 244, 247, 0.8)` frosted glass effect, 16px/500 dark text links, and a right-aligned Shams Yellow pill CTA"
...
- "Design a feature comparison grid with Soft Gray (`#F1F4F7`) background, 24px rounded cards, Shams Yellow checkmark icons, and 14px/700 bold labels"
...
3. Use natural language descriptions, not CSS values — "pill-shaped Shams Yellow button" not "border-radius: 100px; background: #FFC107"
